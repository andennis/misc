...
def test_connect_right_node(root, result):
    data = TreeNodeTraversal.connect_right_nodes_v4(root)
...
def test_serialize_v1(root, result):
    assert TreeNodeTraversal().serialize_v1(root) == result
...
def test_deserialize_v1(tree_data, result):
    root = TreeNodeTraversal().deserialize_v1(tree_data)
    assert TreeNodeTraversal.tree_to_level_list(root) == result
...
def test_serialize_v2(root, result):
    assert TreeNodeTraversal().serialize_v2(root) == result
